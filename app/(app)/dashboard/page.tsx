import { getCurrentUser, getBandHistory, getRecentActivity } from '@/lib/data/users'
...
export default async function DashboardPage() {
  const [user, bandHistory, recentActivity, studyPlan] = await Promise.all([
    getCurrentUser(),
    getBandHistory(),
    getRecentActivity(),
    getStudyPlan('user-1'),
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
        <Card>
          <CardContent className="p-4">
            ...
            <p className="text-3xl font-extrabold text-primary">{formatBand(user.currentBand.overall)}</p>
            <p className="text-xs text-muted-foreground mt-1">Target: {formatBand(user.targetBand)}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4">
        ...
        <Card>
          <CardContent className="p-4">
            ...
            <p className="text-3xl font-extrabold">{completedToday}/{totalToday}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Card>
          <CardHeader className="pb-2">
            ...
            <CardDescription>Current band scores across all four skills</CardDescription>
          </CardHeader>
          <CardContent>
            <BandScoreRadar current={user.currentBand} target={user.targetBand} />
            <div className="grid grid-cols-2 gap-2 mt-4">
              {(['listening', 'reading', 'writing', 'speaking'] as const).map((skill) => (
                <div key={skill} className="flex items-center justify-between rounded-lg bg-muted/50 px-3 py-2 text-sm">
        ...
        <Card>
          <CardHeader className="pb-2">
            ...
            <CardDescription>Band score trajectory — last 6 months</CardDescription>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              ...
              <Link href="/study-plan" className={cn(buttonVariants({ variant: 'ghost', size: 'sm' }), 'gap-1 text-xs')}>
            ...
            {pendingTasks.length === 0 ? (
              ...
              pendingTasks.slice(0, 4).map((task) => {
                ...
                return (
                  ...
                  >
                    <div className={`mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-md text-xs ${skillColor(task.category)}`}>
          ...
          <CardContent className="space-y-2">
            {recentActivity.map((activity) => {
              ...
              return (
                ...
                >
                  <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full ${skillColor(activity.type)}`}>
                  ...
                  {'score' in activity && activity.score && (
                    <Badge variant="secondary" className="text-xs shrink-0">{activity.score}</Badge>
                  )}
