...
export async function fetchListeningTests(page = 1, pageSize = 50) {
  const { data } = await httpClient.get('/api/ielts/listening', { params: { page, pageSize } })
...
export async function fetchReadingTests(page = 1, pageSize = 50) {
  const { data } = await httpClient.get('/api/ielts/reading', { params: { page, pageSize } })
  return (data.data?.items ?? []) as unknown[]
}
...
export async function fetchReadingTest(id: string) {
  const { data: testData } = await httpClient.get(`/api/ielts/reading/${id}`)
  const test = testData.data as Record<string, unknown>

  const { data: sectionsData } = await httpClient.get(`/api/ielts/reading/${id}/sections`, {
  ...
  const sectionsWithQuestions = await Promise.all(
    sections.map(async (section) => {
      const { data: questionsData } = await httpClient.get(`/api/ielts/reading/sections/${section.id}/questions`, {
...
export async function fetchWritingTasks(page = 1, pageSize = 50, type?: string) {
  ...
  const { data } = await httpClient.get('/api/ielts/writing', { params })
  return (data.data?.items ?? []) as unknown[]
}

export async function fetchWritingTask(id: string) {
  const { data } = await httpClient.get(`/api/ielts/writing/${id}`)
  return data.data as unknown
}
...
export async function fetchSpeakingSessions(page = 1, pageSize = 50) {
  const { data } = await httpClient.get('/api/ielts/speaking', { params: { page, pageSize } })
  return (data.data?.items ?? []) as unknown[]
}
...
export async function fetchVocabTopics(page = 1, pageSize = 50, difficulty?: string) {
  ...
  const { data } = await httpClient.get('/api/ielts/vocabulary', { params })
  return (data.data?.items ?? []) as unknown[]
}

export async function fetchVocabTopic(slug: string) {
  const { data } = await httpClient.get(`/api/ielts/vocabulary/${slug}`)
  return data.data as unknown
}
...
export async function fetchMockTests(page = 1, pageSize = 50) {
  const { data } = await httpClient.get('/api/ielts/mock-tests', { params: { page, pageSize } })
  return (data.data?.items ?? []) as unknown[]
}

export async function fetchMockTest(id: string) {
  const { data } = await httpClient.get(`/api/ielts/mock-tests/${id}`)
  return data.data as unknown
}

export async function fetchMockTestSections(id: string, page = 1, pageSize = 20) {
  const { data } = await httpClient.get(`/api/ielts/mock-tests/${id}/sections`, { params: { page, pageSize } })
...
export async function fetchExamGuide() {
  const { data } = await httpClient.get('/api/exam-guide')
  return data.data
}
...
export async function fetchCurrentStudyPlan() {
  const { data } = await httpClient.get('/api/ielts/study-plans/current')
  return data.data
}
...
export async function updateStudyTaskStatus(taskId: string, status: string) {
  const { data } = await httpClient.patch(`/api/ielts/study-plans/tasks/${taskId}`, { status })
  return data.data
}
